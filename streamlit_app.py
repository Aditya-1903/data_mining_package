...
# Load the CNN model
model_path = 'C:\\Users\\Aditya\\OneDrive - CBMM Supply Services and Solutions Pte Ltd\\Desktop\\Data Science\\Data Mining Lab\\final_package\\resnet_keras.h5'
...
# Function to predict top 3 genres
def predict_top3_genre(image_path):
    img = image.load_img(image_path, target_size=(350, 350, 3))
    img = image.img_to_array(img)
    img = img / 255.0
    img = np.expand_dims(img, axis=0)
    y_pred = cnn_model.predict(img)[0]
...
def main():
    ...
    if uploaded_file is not None:
        # Predict genres directly from the uploaded file
        top3_genres = predict_top3_genre(uploaded_file)
        st.image(uploaded_file, caption='Uploaded Image', use_column_width=True)
        st.write("Predicted top 3 genres:", ", ".join(top3_genres))
